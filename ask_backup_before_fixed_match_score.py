...
def evidence_match_score(similarity):
    """
    Benchmark-calibrated evidence strength.

    0.79 is the first cosine level that reached >=90%
    empirical relevance on the 55-question benchmark.
...
def hybrid_search(
    ...
    for (
        ...
        candidates.append({
            ...
            "evidence_match": evidence_match_score(similarity),
            "lexical_score": 0.0,
    ...
    # Relative display score for the Top-K UI.
    # This is NOT cosine similarity and NOT clinical correctness.
    selected = candidates[:top_k]

    if selected:
        top_similarity = selected[0]["similarity"]
        bottom_similarity = selected[-1]["similarity"]
        span = top_similarity - bottom_similarity
        ...
        for item in selected:
            if span <= 1e-9:
                ...
                relative = (
                    item["similarity"] - bottom_similarity
                ) / span

            # Display range: 0.65 .. 0.95
            item["match_score"] = (
                0.65 + 0.30 * relative
            )
